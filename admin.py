...
class App(CTk):
    def __init__(self):
        ...
        self.bg_image_on_canvas = self.canvas.create_image(0, 0, image=self.bg_image_tk, anchor="nw")

        # Creating combobox on the canvas
        self.combo_box = CTkComboBox(self, values=["Prob and Stat", "Comp Org", "Python", "Java"], command=self.change_mode)
        self.combo_box_window = self.canvas.create_window(150, 50, window=self.combo_box)

        # Frame for the table on the canvas
        self.table_frame = CTkFrame(self)
        self.table_frame_window = self.canvas.create_window(150, 180, window=self.table_frame)
